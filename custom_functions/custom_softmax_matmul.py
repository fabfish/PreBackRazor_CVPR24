...
class softmax_matmul(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            ...
            # ZY: move the softmax backward to here to prevent from not requires_grad error
...
class SoftmaxMatMulSparse(nn.Module):
    def __init__(self, args=None, logger=None, quant_groups=1, masker=None, quantize=False, half=False, dim=-1, prebackrazor=False):
