...
class linear_prebackrazor(torch.autograd.Function):
    @staticmethod
    ...
    def backward(ctx, grad_output):
        ...
        # assert ctx.needs_input_grad[0] == True
        ...
        return grad_input, None, grad_bias, None, None, None, None, None, None, None, None, None
    
class linear_postbackrazor(torch.autograd.Function):
    @staticmethod
...
class LinearSparse(nn.Linear, custom_quant.Quant):
    def __init__(self, in_features, out_features, bias=True, args=None, logger=None, quant_groups=1, masker=None,
                 quantize=True, half=False, act_prune=False, prebackrazor=False):
    ...
    def forward(self, x):
        ...
        # print(self.weight.requires_grad,self.bias.requires_grad if self.bias is not None else None)
        if self.masker is not None and self.training:
            if self.prebackrazor == False or self.weight.requires_grad:
                mask = self.masker(x)
                # print("mask sum is {}".format((~mask).sum()))
                if self.act_prune:
                    x = x * mask
                y = linear_postbackrazor.apply(x, self.weight, self.bias, mask, self.quantize, self.half, self.clip_val, self.level,
                                self.iteration, self.ema_decay, self.quant_groups, self.shift)
            else:
                # prebackrazor and weight does not require grad
                y = linear_prebackrazor.apply(x, self.weight, self.bias)
        else:
